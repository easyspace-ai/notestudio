...
func (h *Handler) CreateSession(c *gin.Context) {
	...
	if h.projectService != nil && request.ProjectUUID != "" {
		proj, err := h.projectService.GetProjectByUUID(ctx, request.ProjectUUID)
		if err != nil {
			logger.ErrorWithFields(ctx, err, nil)
			c.Error(errors.NewBadRequestError("invalid project_uuid"))
			return
		}
		pid := proj.ID
		createdSession.ProjectID = &pid
	}
...
func (h *Handler) GetSessionsByTenant(c *gin.Context) {
	...
	switch {
	case projectIDStr != "":
		projectFilter = &projectIDStr
	case projectUUID != "" && h.projectService != nil:
		proj, err := h.projectService.GetProjectByUUID(ctx, projectUUID)
		if err != nil {
			c.Error(errors.NewBadRequestError("invalid project_uuid"))
			return
		}
	...
	result, err := h.sessionService.GetPagedSessionsByTenant(ctx, &pagination, projectFilter)
	if err != nil {
		logger.ErrorWithFields(ctx, err, nil)
		c.Error(errors.NewInternalServerError(err.Error()))
		return
