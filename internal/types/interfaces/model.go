...
type ModelRepository interface {
	...
	// ListPlatformCatalog returns active built-in models (platform-wide catalog for all tenants).
	ListPlatformCatalog(ctx context.Context) ([]*types.Model, error)
	// ListAllPlatformModels returns all built-in model rows (any status) for the operator console.
	ListAllPlatformModels(ctx context.Context) ([]*types.Model, error)
	// UpdatePlatformModel updates a model by ID (platform admin; built-ins allowed).
	UpdatePlatformModel(ctx context.Context, m *types.Model) error
	// GetBuiltInByID returns a built-in model by ID (no tenant scope).
