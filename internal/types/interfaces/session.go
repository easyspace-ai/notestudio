...
type SessionService interface {
	...
	// GetPagedSessionsByTenant gets paged sessions of a tenant; projectID filters by MetaNote project when non-nil.
	GetPagedSessionsByTenant(ctx context.Context, page *types.Pagination, projectID *string) (*types.PageResult, error)
...
type SessionRepository interface {
	...
	// GetPagedByTenantID gets paged sessions of a tenant; projectID filters when non-nil.
	GetPagedByTenantID(ctx context.Context, tenantID uint64, page *types.Pagination, projectID *string) ([]*types.Session, int64, error)
	// ListAllPaged lists sessions across all tenants (platform admin).
	ListAllPaged(ctx context.Context, page *types.Pagination) ([]*types.Session, int64, error)
	// ListSessionIDsByProject returns session IDs for a project (used when deleting a project).
	ListSessionIDsByProject(ctx context.Context, tenantID uint64, projectID string) ([]string, error)
