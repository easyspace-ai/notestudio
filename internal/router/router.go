...
type RouterParams struct {
	...
	PlatformAdminHandler  *handler.PlatformAdminHandler
	PlatformAdminAuth     interfaces.PlatformAdminAuthService
}
...
func NewRouter(params RouterParams) *gin.Engine {
	...
	r.Use(cors.New(cors.Config{
		...
		AllowHeaders:     []string{"Origin", "Content-Type", "Accept", "Accept-Language", "Authorization", "X-API-Key", "X-Request-ID", "X-Tenant-ID"},
	...
	// 认证中间件
	r.Use(middleware.Auth(params.TenantService, params.UserService, params.Config, params.PlatformAdminAuth))

	// 文件服务：统一代理本地/MinIO/COS/TOS存储后端（需要认证）
	serveFiles(r, params.Config)
	...
	v1 := r.Group("/api/v1")
	{
		...
		RegisterPlatformAdminRoutes(v1, params.PlatformAdminHandler)
		...
		RegisterStudioRoutes(v1, params.StudioHandler)
	}
...
// RegisterPlatformAdminRoutes registers SaaS operator (platform admin) routes.
func RegisterPlatformAdminRoutes(r *gin.RouterGroup, h *handler.PlatformAdminHandler) {
	r.POST("/admin/auth/login", h.Login)
	r.GET("/admin/models", h.ListPlatformModels)
	r.POST("/admin/models", h.CreatePlatformModel)
	r.PUT("/admin/models/:id", h.UpdatePlatformModel)
	r.DELETE("/admin/models/:id", h.DeletePlatformModel)
	r.GET("/admin/knowledge-bases", h.ListAllKnowledgeBases)
	r.GET("/admin/sessions", h.ListAllSessions)
	r.GET("/admin/skills", h.ListAdminSkills)
...
func serveFiles(r *gin.Engine, cfg *config.Config) {
	...
	r.GET("/files", func(c *gin.Context) {
		...
		sec := config.EffectiveStorageEngine(cfg, tenant)
		if sec == nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "storage engine not configured (platform_storage_engine or tenant)"})
			return
		}
...
// RegisterProjectRoutes registers MetaNote project routes.
func RegisterProjectRoutes(r *gin.RouterGroup, h *handler.ProjectHandler) {
	if h == nil {
		return
	}
	projects := r.Group("/projects")
	{
		...
		projects.GET("/by-uuid/:uuid", h.GetProjectByUUID)
...
// RegisterStudioRoutes registers MetaNote Studio async job APIs.
func RegisterStudioRoutes(r *gin.RouterGroup, h *handler.StudioHandler) {
	if h == nil {
		return
	}
	studio := r.Group("/studio")
	{
		studio.POST("/jobs", h.CreateJob)
		studio.GET("/jobs", h.ListJobs)
		studio.GET("/jobs/:id", h.GetJob)
	}
}
