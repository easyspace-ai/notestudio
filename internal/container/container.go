...
func BuildContainer(container *dig.Container) *dig.Container {
	...
	must(container.Provide(repository.NewStudioJobRepository))
	must(container.Provide(repository.NewMessageRepository))
	must(container.Provide(repository.NewModelRepository))
	must(container.Provide(repository.NewPlatformAdminRepository))
	must(container.Provide(service.NewPlatformAdminAuthService))
	...
	must(container.Provide(service.NewStudioService))
	must(container.Provide(service.NewStudioTaskHandler, dig.Name("studioWorker")))
	...
	must(container.Provide(handler.NewStudioHandler))
	must(container.Provide(handler.NewMessageHandler))
	must(container.Provide(handler.NewModelHandler))
	must(container.Provide(handler.NewEvaluationHandler))
	must(container.Provide(handler.NewInitializationHandler))
	must(container.Provide(handler.NewAuthHandler))
	must(container.Provide(handler.NewPlatformAdminHandler))
	...
	must(container.Invoke(func(svc interfaces.PlatformAdminAuthService) error {
		seedCtx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
		return svc.EnsureDefaultAdmin(seedCtx)
	}))
