...
type sessionService struct {
	...
	skillService          interfaces.SkillService                  // Skills discovery / overrides for agent QA
}
...
func (s *sessionService) GetPagedSessionsByTenant(ctx context.Context,
	...
	sessions, total, err := s.sessionRepo.GetPagedByTenantID(ctx, tenantID, pagination, projectID)
	if err != nil {
		logger.ErrorWithFields(ctx, err, map[string]interface{}{
			"tenant_id": tenantID,
