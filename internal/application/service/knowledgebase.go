...
// applyKnowledgeBaseCreationDefaults fills missing embedding/summary/VLM from the platform catalog
// (ListModels: active rows with is_builtin = true only). Prefer built-in + is_default, then any active builtin.
// When the KB has no storage_provider_config, copies the tenant's storage default_provider so uploads and
// the UI agree with platform-wide storage settings.
// Caller-supplied kb fields win when non-empty.
func (s *knowledgeBaseService) applyKnowledgeBaseCreationDefaults(ctx context.Context, kb *types.KnowledgeBase) error {
	models, err := s.modelService.ListModels(ctx)
	if err != nil {
		return fmt.Errorf("list models for KB defaults: %w", err)
	}
	builtinModels := make([]*types.Model, 0, len(models))
	...
	if len(builtinModels) == 0 {
		return errors.New("no active built-in models: platform admin must configure models with is_builtin=true and status=active")
	}
	if strings.TrimSpace(kb.EmbeddingModelID) == "" {
		kb.EmbeddingModelID = PickPreferredActiveModelID(builtinModels, types.ModelTypeEmbedding)
	}
	if strings.TrimSpace(kb.SummaryModelID) == "" {
		kb.SummaryModelID = PickPreferredActiveModelID(builtinModels, types.ModelTypeKnowledgeQA)
	}
	if kb.VLMConfig.Enabled && strings.TrimSpace(kb.VLMConfig.ModelID) == "" {
		kb.VLMConfig.ModelID = PickPreferredActiveModelID(builtinModels, types.ModelTypeVLLM)
	}
	if strings.TrimSpace(kb.EmbeddingModelID) == "" || strings.TrimSpace(kb.SummaryModelID) == "" {
		return errors.New("embedding_model_id and summary_model_id are required: add active built-in Embedding and KnowledgeQA models (is_builtin=true), or pass IDs in the request")
	}
	s.applyStorageProviderDefault(ctx, kb)
	return nil
}

// applyStorageProviderDefault sets KB storage provider when unset: platform_storage_engine first, then tenant default.
func (s *knowledgeBaseService) applyStorageProviderDefault(ctx context.Context, kb *types.KnowledgeBase) {
	if kb == nil || kb.GetStorageProvider() != "" {
		return
	}
	if s.cfg != nil && s.cfg.PlatformStorageEngineActive() {
		kb.SetStorageProvider(strings.ToLower(strings.TrimSpace(s.cfg.PlatformStorageEngine.DefaultProvider)))
		return
	}
	var p string
	if tenant, ok := types.TenantInfoFromContext(ctx); ok && tenant != nil && tenant.StorageEngineConfig != nil {
		p = strings.ToLower(strings.TrimSpace(tenant.StorageEngineConfig.DefaultProvider))
	}
	if p == "" && s.tenantRepo != nil {
		tid := kb.TenantID
		if tid == 0 {
			tid = types.MustTenantIDFromContext(ctx)
		}
		if t, err := s.tenantRepo.GetTenantByID(ctx, tid); err == nil && t != nil && t.StorageEngineConfig != nil {
			p = strings.ToLower(strings.TrimSpace(t.StorageEngineConfig.DefaultProvider))
		}
	}
	if p != "" {
