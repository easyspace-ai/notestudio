...
// checkStorageEngineConfigured verifies platform/tenant/KB effective storage is set for uploads.
func (s *knowledgeService) checkStorageEngineConfigured(ctx context.Context, kb *types.KnowledgeBase) error {
	tenant, _ := ctx.Value(types.TenantInfoContextKey).(*types.Tenant)
	p := config.EffectiveStorageProvider(s.config, kb, tenant)
	if p == "" {
		return werrors.NewBadRequestError("未配置存储引擎：请在 config.yaml 的 platform_storage_engine 中设置 default_provider 及对应引擎参数，或在租户设置中配置默认存储引擎后再上传。")
	}
	return nil
}
...
func (s *knowledgeService) CreateKnowledgeFromFile(ctx context.Context,
	...
	if !IsImageType(getFileType(fileName)) {
		...
		tenant, _ := ctx.Value(types.TenantInfoContextKey).(*types.Tenant)
		provider := config.EffectiveStorageProvider(s.config, kb, tenant)
		sec := config.EffectiveStorageEngine(s.config, tenant)

		// 根据 provider 校验有效存储引擎配置（平台级优先）
		switch provider {
		case "cos":
			if sec == nil || sec.COS == nil ||
				sec.COS.SecretID == "" || sec.COS.SecretKey == "" ||
				sec.COS.Region == "" || sec.COS.BucketName == "" {
				logger.Error(ctx, "COS configuration incomplete for image multimodal processing")
				return nil, werrors.NewBadRequestError("上传图片文件需要完整的对象存储配置信息, 请在 platform_storage_engine 或租户存储设置中补全 COS 参数")
			}
		case "minio":
			ok := false
			...
			if !ok {
				...
				return nil, werrors.NewBadRequestError("上传图片文件需要完整的对象存储配置信息, 请在 platform_storage_engine 或租户存储设置中补全 MinIO 参数")
			}
		}
...
func (s *knowledgeService) buildStorageConfig(ctx context.Context, kb *types.KnowledgeBase) *types.DocParserStorageConfig {
	...
	// Merge from platform or tenant StorageEngineConfig.
	...
	logger.Infof(ctx, "[storage] buildStorageConfig use merged platform/tenant config: kb=%s provider=%s bucket=%s path_prefix=%s endpoint=%s",
...
// using platform_storage_engine when configured, else KB + tenant StorageEngineConfig.
// Falls back to the global fileSvc when no storage config is found.
func (s *knowledgeService) resolveFileService(ctx context.Context, kb *types.KnowledgeBase) interfaces.FileService {
	...
	if provider == "" || sec == nil {
		logger.Infof(ctx, "[storage] resolveFileService fallback default: kb=%s provider=%q platform=%v tenant_cfg=%v",
			kb.ID, provider, s.config != nil && s.config.PlatformStorageEngineActive(), tenant != nil && tenant.StorageEngineConfig != nil)
		return s.fileSvc
	}
...
func (s *knowledgeService) resolveFileServiceForPath(ctx context.Context, kb *types.KnowledgeBase, filePath string) interfaces.FileService {
	...
	tenant, _ := ctx.Value(types.TenantInfoContextKey).(*types.Tenant)
	configured := config.EffectiveStorageProvider(s.config, kb, tenant)
	if configured == "" {
