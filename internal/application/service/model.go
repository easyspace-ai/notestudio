...
// ListModels returns the SaaS platform catalog: active built-in models only (shared by all tenants).
func (s *modelService) ListModels(ctx context.Context) ([]*types.Model, error) {
	logger.Info(ctx, "Start listing platform model catalog")

	models, err := s.repo.ListPlatformCatalog(ctx)
	if err != nil {
		logger.ErrorWithFields(ctx, err, nil)
		return nil, err
	}

	logger.Infof(ctx, "Retrieved %d platform models", len(models))
	return models, nil
}
