...
func (s *agentService) CreateAgentEngine(
	...
	taskRunner := s.newTaskRunner(config, chatModel, rerankModel, sessionID)
	if err := s.registerTools(ctx, toolRegistry, config, rerankModel, chatModel, sessionID, taskRunner); err != nil {
...
func (s *agentService) registerTools(
	...
	for _, toolName := range allowedTools {
		...
		case tools.ToolReadFile:
			toolToRegister = tools.NewReadFileTool(config.ReadFileOutputMaxChars)
		case tools.ToolWriteFile:
			toolToRegister = tools.NewWriteFileTool()
		case tools.ToolStrReplace:
			toolToRegister = tools.NewStrReplaceTool()
		case tools.ToolLs:
			toolToRegister = tools.NewLsTool()
		case tools.ToolGlob:
			toolToRegister = tools.NewGlobTool()
		case tools.ToolBash:
			toolToRegister = tools.NewBashTool(config.BashOutputMaxChars)
		case tools.ToolTask:
			if taskRunner != nil {
				toolToRegister = tools.NewTaskTool(taskRunner)
			} else {
				logger.Warnf(ctx, "Skipping task tool: no runner")
			}
		case tools.ToolViewImage:
			toolToRegister = tools.NewViewImageTool()
		case tools.ToolToolSearch:
			toolToRegister = tools.NewToolSearchTool()
		case tools.ToolInvokeACPAgent:
			toolToRegister = tools.NewInvokeACPAgentTool()
		case tools.ToolSetupAgent:
			toolToRegister = tools.NewSetupAgentTool()
		default:
