...
func (s *sessionService) buildAgentConfig(
	...
	agentConfig := &types.AgentConfig{
		...
		ReadFileOutputMaxChars:      customAgent.Config.ReadFileOutputMaxChars,
		BashOutputMaxChars:          customAgent.Config.BashOutputMaxChars,
	}
...
func (s *sessionService) configureSkillsFromAgent(
	...
	case "all":
		...
		if err != nil {
			logger.Warnf(ctx, "ResolveAgentSkillAllowlist failed: %v", err)
		...
		if disableAll {
			...
			agentConfig.SkillDirs = nil
			agentConfig.AllowedSkills = nil
			logger.Infof(ctx, "SkillsSelectionMode=all: no enabled skills (all disabled in overrides)")
			break
		}
		agentConfig.SkillsEnabled = true
		agentConfig.SkillDirs = dirs
		agentConfig.AllowedSkills = allowed
		logger.Infof(ctx, "SkillsSelectionMode=all: skillDirs=%v allowed=%v", dirs, allowed)
	case "selected":
		...
		filtered, err := s.skillService.FilterToEnabledSkillNames(ctx, customAgent.Config.SelectedSkills)
		if err != nil {
			logger.Warnf(ctx, "FilterToEnabledSkillNames failed: %v", err)
			agentConfig.SkillsEnabled = false
			agentConfig.SkillDirs = nil
			agentConfig.AllowedSkills = nil
			break
		...
		if len(filtered) == 0 {
			...
			agentConfig.SkillDirs = nil
			agentConfig.AllowedSkills = nil
			logger.Infof(ctx, "SkillsSelectionMode=selected: no selected skills remain after disable filter")
			break
		}
		agentConfig.SkillsEnabled = true
		agentConfig.SkillDirs = DefaultSkillParentDirs()
		agentConfig.AllowedSkills = filtered
		logger.Infof(ctx, "SkillsSelectionMode=selected: enabled %d skills: %v", len(filtered), filtered)
	case "none", "":
		// Skills disabled
		agentConfig.SkillsEnabled = false
