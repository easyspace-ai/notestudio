...
func (s *ImageMultimodalService) resolveFileServiceForPayload(ctx context.Context, payload types.ImageMultimodalPayload) interfaces.FileService {
	...
	if provider == "" {
		var kbPtr *types.KnowledgeBase
		if kb != nil {
			kbPtr = kb
		}
		provider = config.EffectiveStorageProvider(s.cfg, kbPtr, tenant)
	}

	sec := config.EffectiveStorageEngine(s.cfg, tenant)
	if sec == nil || provider == "" {
		logger.Warnf(ctx, "[ImageMultimodal] missing storage config: tenant=%d provider=%q", payload.TenantID, provider)
		return nil
	}

	baseDir := strings.TrimSpace(os.Getenv("LOCAL_STORAGE_BASE_DIR"))
	fileSvc, _, svcErr := filesvc.NewFileServiceFromStorageConfig(provider, sec, baseDir)
	if svcErr != nil {
