...
// ListAllNonTemporaryForAdmin lists all non-ephemeral knowledge bases across tenants.
func (r *knowledgeBaseRepository) ListAllNonTemporaryForAdmin(ctx context.Context) ([]*types.KnowledgeBase, error) {
	var kbs []*types.KnowledgeBase
	err := r.db.WithContext(ctx).Where("is_temporary = ?", false).
		Order("created_at DESC").Find(&kbs).Error
	if err != nil {
		return nil, err
	}
	return kbs, nil
