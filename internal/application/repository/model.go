...
// ListPlatformCatalog returns active built-in models (SaaS platform catalog for all tenants).
func (r *modelRepository) ListPlatformCatalog(ctx context.Context) ([]*types.Model, error) {
	var models []*types.Model
	err := r.db.WithContext(ctx).
		Where("is_builtin = ? AND status = ?", true, types.ModelStatusActive).
		Order("type ASC, name ASC").
		Find(&models).Error
	return models, err
}

// ListAllPlatformModels returns built-in models in any status (operator console).
func (r *modelRepository) ListAllPlatformModels(ctx context.Context) ([]*types.Model, error) {
	var models []*types.Model
	err := r.db.WithContext(ctx).Where("is_builtin = ?", true).Order("type ASC, name ASC").Find(&models).Error
	return models, err
}

// UpdatePlatformModel persists a full model row (platform admin; includes built-ins).
func (r *modelRepository) UpdatePlatformModel(ctx context.Context, m *types.Model) error {
	return r.db.WithContext(ctx).Save(m).Error
}

// GetBuiltInByID returns a built-in model by ID.
func (r *modelRepository) GetBuiltInByID(ctx context.Context, id string) (*types.Model, error) {
	var m types.Model
	err := r.db.WithContext(ctx).Where("id = ? AND is_builtin = ?", id, true).First(&m).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, nil
	}
	if err != nil {
