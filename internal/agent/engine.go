...
func (e *AgentEngine) Execute(
	...
	// Extract user language from context for prompt placeholder; align with user message when locale is wrong.
	language := types.EffectiveOutputLanguageName(ctx, query)
	...
	systemPrompt = types.AppendOutputLanguageDirective(systemPrompt, language)
