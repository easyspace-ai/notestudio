...
var toolDisplayNames = map[string]string{
	...
	agenttools.ToolWriteFile:           "写入文件",
	agenttools.ToolStrReplace:          "替换文件",
	agenttools.ToolLs:                  "列出目录",
	agenttools.ToolGlob:                "文件匹配",
	agenttools.ToolBash:                "Shell",
	agenttools.ToolTask:                "子任务",
	agenttools.ToolViewImage:           "查看图片",
	agenttools.ToolToolSearch:          "工具搜索",
	agenttools.ToolInvokeACPAgent:      "外部代理",
	agenttools.ToolSetupAgent:          "保存智能体草稿",
}
...
func (e *AgentEngine) runToolCall(
	...
	toolBaseCtx := ctx
	if tid, ok := types.SessionTenantIDFromContext(ctx); ok {
		toolBaseCtx = workspace.WithTenantID(toolBaseCtx, tid)
	} else if tid, ok := types.TenantIDFromContext(ctx); ok {
		toolBaseCtx = workspace.WithTenantID(toolBaseCtx, tid)
	}
	toolBaseCtx = workspace.WithSessionID(toolBaseCtx, sessionID)
	if e.config != nil {
		toolBaseCtx = workspace.WithSkillDirs(toolBaseCtx, e.config.SkillDirs)
	}

	toolCtx, toolCancel := context.WithTimeout(toolBaseCtx, defaultToolExecTimeout)
