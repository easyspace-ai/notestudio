...
export const useAuthStore = defineStore('auth', () => {
  ...
  /** 平台管理员 JWT（与终端用户 weknora_token 独立） */
  const platformAdminToken = ref<string>('')
  const platformAdmin = ref<PlatformAdminInfo | null>(null)
  ...
  const isLoggedIn = computed(() => {
    if (isPlatformConsole()) {
      // 平台控制台仅依赖平台管理员 token；admin 基本信息偶发丢失不应导致强制登出
      return !!platformAdminToken.value
    }
  ...
  const setPlatformAdminSession = (accessToken: string, admin: PlatformAdminInfo) => {
    platformAdminToken.value = accessToken
    platformAdmin.value = admin
    localStorage.setItem('weknora_platform_admin_token', accessToken)
    localStorage.setItem('weknora_platform_admin', JSON.stringify(admin))
  }

  /** 仅清除终端用户登录态，不影响平台管理员 token（用于管理员登录前清理残留） */
  const clearEndUserSession = () => {
    user.value = null
    tenant.value = null
    token.value = ''
    refreshToken.value = ''
  ...
  const initFromStorage = () => {
    ...
    const storedPlatformAdminToken = localStorage.getItem('weknora_platform_admin_token')
    const storedPlatformAdmin = localStorage.getItem('weknora_platform_admin')
    ...
    if (storedPlatformAdminToken) {
      platformAdminToken.value = storedPlatformAdminToken
    }
    if (storedPlatformAdmin) {
      try {
        platformAdmin.value = JSON.parse(storedPlatformAdmin) as PlatformAdminInfo
      } catch (e) {
        console.error('Failed to parse platform admin', e)
      }
    }
