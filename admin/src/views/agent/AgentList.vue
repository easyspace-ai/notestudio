      ...
      <div v-if="sharedDetailVisible && currentSharedAgent" class="shared-detail-drawer-overlay" @click.self="closeSharedAgentDetail">
        <div class="shared-detail-drawer">
          ...
          <div class="shared-detail-drawer-footer">
            <t-button
              v-if="!isPlatformManagePage"
              theme="primary"
              block
              @click="handleUseSharedAgentInChat(currentSharedAgent)"
            >
            ...
            <t-alert
              v-else
              theme="info"
              message="平台管理页面中无法直接使用 Agent，请返回主界面操作"
            />
          </div>
        </div>
      </div>
...
// 检查是否在平台管理页面中
const isPlatformManagePage = computed(() => {
  return route.path.startsWith('/platform/manage/')
})
...
const handleCardClick = (agent: DisplayAgent | AgentWithUI) => {
  // 关闭已打开的「更多」菜单，避免菜单展开时点击卡片被误判为仅关菜单而不进入编辑/详情
  openMoreAgentId.value = null
