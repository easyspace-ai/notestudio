...
const handleMsgList = async (data, isScrollType = false, newScrollHeight) => {
    ...
    for (let i = 0, len = chatlist.length; i < len; i++) {
        ...
        if (item.content) {
            if (!item.content.includes('<redacted_thinking>') && !item.content.includes('<\/redacted_thinking>')) {
                item.thinkContent = "";
                item.content = item.content;
                item.showThink = false;
                item.thinking = false;
            } else if (item.content.includes('<\/redacted_thinking>')) {
                // 历史消息中包含完整的 <think>...</think> 标签，说明 thinking 已完成
                item.showThink = true;
                item.thinking = false;  // 关键：标记 thinking 已完成，使 deepThink 默认折叠
                const index = item.content.trim().lastIndexOf('<\/redacted_thinking>');
                item.thinkContent = item.content.trim().substring(0, index).replace('<redacted_thinking>', '').trim();
                item.content = item.content.trim().substring(index + '</redacted_thinking>'.length);
            } else if (item.content.includes('<redacted_thinking>')) {
                // 内容包含 <think> 但没有 </think>，说明 thinking 还在进行中（不太可能出现在历史消息中）
                item.showThink = true;
                item.thinking = true;
                item.thinkContent = item.content.replace('<redacted_thinking>', '').trim();
                item.content = '';
            }
        }
