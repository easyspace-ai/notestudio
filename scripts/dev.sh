...
# 递归结束进程树（用于 Ctrl+C 后清理 air/go build 等子进程）
...
start_services() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [[ "$ENABLED_SERVICES" == *"minio"* ]]; then
            echo "  - MinIO:         localhost:6370 (Console: localhost:9001)"
        fi
...
start_app() {
    ...
    # 本地进程连 compose：仅填补未设置的变量，或把 Docker 服务名换成本机。
    # 切勿无条件覆盖 .env（例如 MINIO/Redis 自定义映射端口会被错误改成 9000/6379）。
    case "${DB_HOST:-}" in
        ""|postgres|app) export DB_HOST=localhost ;;
    esac
    [[ -n "${DOCREADER_ADDR:-}" ]] || export DOCREADER_ADDR=localhost:50051
    [[ -n "${DOCREADER_TRANSPORT:-}" ]] || export DOCREADER_TRANSPORT=grpc
    case "${MINIO_ENDPOINT:-}" in
        ""|minio:9000) export MINIO_ENDPOINT=localhost:9000 ;;
    esac
    case "${REDIS_ADDR:-}" in
    ...
    [[ -n "${QDRANT_HOST:-}" ]] || export QDRANT_HOST=localhost
    ...
    if command -v air &> /dev/null; then
        ...
        app_cmd=(go run -ldflags="$LDFLAGS" cmd/server/main.go)
    fi
    ...
    cleanup_app() {
        ...
        if kill -0 "$app_pid" 2>/dev/null; then
            log_info "收到退出信号，正在停止后端进程..."
            terminate_process_tree "$app_pid" "INT"
            sleep 0.2
    ...
    trap cleanup_app INT TERM EXIT
    wait "$app_pid"
    local app_exit_code=$?
    trap - INT TERM EXIT
